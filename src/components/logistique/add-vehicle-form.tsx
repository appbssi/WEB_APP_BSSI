...
export function AddVehicleForm({ onSuccess }: { onSuccess: () => void }) {
  ...
  const form = useForm<z.infer<typeof vehicleSchema>>({
    ...
    defaultValues: { plateNumber: '', model: '', type: 'Pick-up', mileage: 0, nextMaintenanceMileage: 5000 },
  });
  ...
  const onSubmit = async (values: z.infer<typeof vehicleSchema>) => {
    ...
    try {
      ...
      logActivity(firestore, `Nouveau véhicule ajouté : ${values.model} (${values.plateNumber})`, 'Logistique', '/logistique');
      toast({ title: 'Véhicule enregistré' });
      onSuccess();
    } catch (error) {
      toast({ variant: 'destructive', title: 'Erreur', description: "Impossible d'enregistrer le véhicule." });
    }
  };
